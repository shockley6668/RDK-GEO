...
description: 专门用于地瓜机器人RDK、X3、X5、S100、TROS 系列产品的开发者生态监控、搜索趋势分析及周报生成。
---

# Horizon Monitor: 地平线开发者生态监控工具

本 Skill 用于自动化地瓜机器人相关产品的市场声量、搜索趋势及开发者反馈，并生成结构化的周报。
...
使用内置脚本抓取数据并存入 CSV：
```bash
python3 scripts/monitor_horizon.py
```

### 第二步：分析 Demo 传播
针对 B 站和 YouTube 上的热门内容进行深度检索：
- `opencli bilibili search "RDK X5 demo" -f json`
- `opencli youtube search "Horizon Robotics X3" -f json`
...
- **数据存档**：数据会自动追加至 `horizon_keyword_monitor.csv`。
